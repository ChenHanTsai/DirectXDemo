...
float4 main(V_OUT input) : SV_TARGET
{
	...
	bumpMap = shaderTexture[2].Sample(SampleType, input.tex);//debug
	//bumpMap = shaderTexture[0].Sample(SampleType, input.tex);
	...
	//clamp
//	lightRatio = saturate(dot(lightDirCal, input.nor)); debug
	...
	if (lightRatio > 0)
	{
		...
		//reflectDir = normalize(2 * lightRatio*input.nor - lightDirCal); //debug
	...
	//	spotLightRatio = saturate(dot(input.spotLightPosVOut, input.nor));
		spotLightRatio = saturate(dot(input.spotLightPosVOut, bumpNormal));//debug
	}
	else
	{
		spotLightRatio = 0.0f;
